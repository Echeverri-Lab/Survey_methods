...
text = """
Value-Belief-Norm (VBN) Theory (Stern et al., 1999) — arguably the most cited theory in environmental behavior research; links ecological worldviews → beliefs → moral norms → action
...
print("| Theory / Framework | Main Argument | Primary Author(s) & Year |")
print("| :--- | :--- | :--- |")
...
for line in lines:
    if line.strip():
        parts = line.split(" — ")
        if len(parts) >= 2:
            theory_author = parts[0].strip()
            argument = parts[1].strip()
            
            # extract author
            match = re.search(r'\((.*?)\)$', theory_author)
            if match:
                author_year = match.group(1)
                theory = theory_author[:match.start()].strip()
            else:
                author_year = "N/A"
                theory = theory_author
                
            print(f"| **{theory}** | {argument} | {author_year} |")
